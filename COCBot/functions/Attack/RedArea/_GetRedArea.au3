...
Func _GetRedArea($iMode = $REDLINE_IMGLOC, $iMaxAllowedPixelDistance = 25, $fMinSideLengthFactor = 0.65)
	...
	If $g_iMatchMode = $RankedBattle And $g_aiAttackAlgorithm[$RankedBattle] = 0 And $g_aiAttackStdDropSides[$RankedBattle] = 4 Then ; Used for DES Side Attack (need to know the side the DES is on)
	...
	ElseIf $g_iMatchMode = $RankedBattle And $g_aiAttackAlgorithm[$RankedBattle] = 0 And $g_aiAttackStdDropSides[$RankedBattle] = 5 Then ; Used for TH Side Attack (need to know the side the TH is on)
